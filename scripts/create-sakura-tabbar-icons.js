...
// 樱花粉色
const SAKURA_PINK = 0xF48FB1FF;
const SAKURA_DARK = 0xC2185BFF;
...
async function generateIcons() {
  ...
  for (const icon of icons) {
    ...
    icon.draw(activeImage, SAKURA_PINK);
    await activeImage.writeAsync(path.join(outputDir, `${icon.name}-active.png`));

    console.log(`生成樱花图标: ${icon.name}.png 和 ${icon.name}-active.png`);
  }

  console.log('所有樱花图标生成完成！');
}
